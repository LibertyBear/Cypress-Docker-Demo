...
pipeline {
    ...
    stages {
        ...
        stage('Testing') {
            steps {
                echo "Testing..."
                sh "docker run -v '$PWD':/my-cypress-project -t my-cypress-image:1.1.0 --spec cypress/e2e/pom/homeSauce.spec.js --headless --browser chrome --disable-gpu --config-file=/my-cypress-project/cypress.config.ts"
